...
// Attempt to parse dollars, adding either a dollar math span or a
// literal sequence of dollars.
var parseDollars = function(block) {
    var dollars = this.match(reDollarsHere);
    if (dollars === null) {
        return false;
    }
    var afterOpenDollars = this.pos;
    var matched;
    var node;
    while ((matched = this.match(reDollars)) !== null) {
		...
		node = new Node('MathBlock');
	    }
            node._literal = this.subject.slice(
		afterOpenDollars,
                this.pos - dollars.length)
                .trim().replace(reWhitespace, ' ');
            block.appendChild(node);
            return true;
        }
    ...
    this.pos = afterOpenDollars;
    block.appendChild(text(dollars));
    return true;
};
